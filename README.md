...
- **Student Registry** — Console app that manages a student registry using List<Student>. Features add, search, display all, and display passing students (grade >= 75). First use of FindAll() for list filtering and computed properties for auto-calculating pass/failgi status based on grade.
